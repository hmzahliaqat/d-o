...
public emailPassword = {
  signIn: async <
    ExtraFields extends Record<string, unknown> = {}
  >(
    data: Omit<TEmailPasswordSignin, 'csrfToken'> & { csrfToken?: string } & ExtraFields
  ) => {
    let csrfToken = data.csrfToken;

    if (!csrfToken) {
      csrfToken = (await this.client.csrf.$get().then(res => res.json())).csrfToken;
    }
  ...
  signUp: async <
    ExtraFields extends Record<string, unknown> = {}
  >(
    data: Omit<TSignUpSchema, 'csrfToken'> & { csrfToken?: string } & ExtraFields
  ) => {
    let csrfToken = data.csrfToken;
    ...
    const response = await this.client['email-password']['signup'].$post({
      json: {
        ...data,
        csrfToken,
      },
  ...
  resendVerifyEmail: async (data: TResendVerifyEmailSchema) => {
    const response = await this.client['email-password']['resend-verify-email'].$post({ json: data });
    await this.handleError(response);
  },
